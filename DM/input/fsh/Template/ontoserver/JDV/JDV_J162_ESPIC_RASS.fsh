ValueSet: JDV_J162_ESPIC_RASS
Id: JDV-J162-ESPIC-RASS
Description: "Etablissement de santé privée d'intérêt collectif dans le RASS"
* ^meta.versionId = "5"
* ^meta.lastUpdated = "2026-07-06T19:05:19.582+02:00"
* ^meta.profile = "http://hl7.org/fhir/StructureDefinition/shareablevalueset"
* ^language = #fr-FR
* ^extension.url = "http://hl7.org/fhir/StructureDefinition/resource-effectivePeriod"
* ^extension.valuePeriod.start = "2021-05-28T12:00:00+01:00"
* ^url = "https://mos.esante.gouv.fr/NOS/JDV_J162-ESPIC-RASS/FHIR/JDV-J162-ESPIC-RASS"
* ^identifier.system = "urn:ietf:rfc:3986"
* ^identifier.value = "urn:oid:1.2.250.1.213.1.6.1.226"
* ^version = "20210528120000"
* ^status = #active
* ^experimental = false
* ^date = "2021-05-28T12:00:00+01:00"
* ^publisher = "Agence du Numérique en Santé (ANS) - 2-10 Rue d'Oradour-sur-Glane, 75015 Paris"
* ^jurisdiction = urn:iso:std:iso:3166#FRA
* $TRE-R73-ESPIC#0 "Non concerné"
* $TRE-R73-ESPIC#1 "Etablissement public de santé"
* $TRE-R73-ESPIC#2 "PSPH par intégration"
* $TRE-R73-ESPIC#3 "PSPH par concession"
* $TRE-R73-ESPIC#4 "PSPH par association"
* $TRE-R73-ESPIC#5 "Assoc 1901 act. Psy"
* $TRE-R73-ESPIC#6 "Etablissement de santé privé d'intérêt collectif"
* $TRE-R73-ESPIC#7 "Etablissement de santé privé non lucratif, non déclaré d'intérêt collectif"
* $TRE-R73-ESPIC#9 "Indéterminé"